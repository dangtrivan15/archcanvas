...
describe('registryStore', () => {
  ...
  describe('checkForUpdates', () => {
    it('sets availableUpdates when the registry returns newer versions', async () => {
      vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
        ok: true,
        json: async () => ({
          updates: [
      ...
      useRegistryStore.setState({
        remoteInstalledVersions: new Map([['community/kubernetes-deployment', '1.0.0']]),
      });

      await useRegistryStore.getState().checkForUpdates();

      const state = useRegistryStore.getState();
      expect(state.availableUpdates.get('community/kubernetes-deployment')).toBe('2.0.0');
      expect(state.updatesCheckStatus).toBe('done');
    });

    it('is a no-op when there are no remote entries installed', async () => {
      const fetchMock = vi.fn();
      vi.stubGlobal('fetch', fetchMock);

      useRegistryStore.setState({ remoteInstalledVersions: new Map() });

      await useRegistryStore.getState().checkForUpdates();

      expect(fetchMock).not.toHaveBeenCalled();
      expect(useRegistryStore.getState().availableUpdates.size).toBe(0);
    });
  });

  describe('applyUpdate', () => {
    it('downloads, reloads, and removes the entry from availableUpdates', async () => {
      const fs = new InMemoryFileSystem();

      // Pre-seed with a remote nodedef
      const lockfileYaml = [
        '# Auto-generated by ArchCanvas',
        'lockfileVersion: 1',
        "resolvedAt: '2026-01-01T00:00:00.000Z'",
...
`;
      ...
      useRegistryStore.setState({
        availableUpdates: new Map([['community/kubernetes-deployment', '2.0.0']]),
        pinnedVersions: new Map(),
      });
      ...
      expect(state.pinnedVersions.has('community/kubernetes-deployment')).toBe(false);
    });
  });

  describe('dismissUpdate', () => {
    it('adds the entry to pinnedVersions and persists to localStorage', () => {
      localStorageMock.clear();
      useRegistryStore.setState({ pinnedVersions: new Map() });

      useRegistryStore.getState().dismissUpdate('community/kubernetes-deployment', '2.0.0');

      const state = useRegistryStore.getState();
      expect(state.pinnedVersions.get('community/kubernetes-deployment')).toBe('2.0.0');

      const stored = JSON.parse(localStorageMock.getItem('archcanvas:registry-pinned-versions') ?? '{}');
      expect(stored['community/kubernetes-deployment']).toBe('2.0.0');
    });
  });

  describe('effectiveUpdateCount', () => {
    it('excludes pinned entries from update count', () => {
      useRegistryStore.setState({
        availableUpdates: new Map([
          ['ns/a', '2.0.0'],
          ['ns/b', '3.0.0'],
        ]),
        pinnedVersions: new Map([['ns/a', '2.0.0']]), // ns/a is pinned to latest
      });
    ...
    it('re-shows badge after newer version available despite existing pin', () => {
      useRegistryStore.setState({
        availableUpdates: new Map([
          ['ns/a', '3.0.0'], // registry now has v3
        ]),
        pinnedVersions: new Map([['ns/a', '2.0.0']]), // user dismissed v2
      });
