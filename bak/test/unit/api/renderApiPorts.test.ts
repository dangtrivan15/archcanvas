...
describe('RenderApi generates port handles from nodedef spec', () => {
  ...
  it('compute/service port protocols match nodedef spec', () => {
    ...
    const httpIn = canvasNode.data.ports.inbound.find((p) => p.name === 'http-in');
    expect(httpIn).toBeDefined();
    expect(httpIn!.protocol).toEqual(['HTTP', 'HTTPS']);

    const grpcIn = canvasNode.data.ports.inbound.find((p) => p.name === 'grpc-in');
    expect(grpcIn).toBeDefined();
    expect(grpcIn!.protocol).toEqual(['gRPC']);

    const httpOut = canvasNode.data.ports.outbound.find((p) => p.name === 'http-out');
    expect(httpOut).toBeDefined();
    expect(httpOut!.protocol).toEqual(['HTTP', 'HTTPS']);

    const grpcOut = canvasNode.data.ports.outbound.find((p) => p.name === 'grpc-out');
  ...
  it('data/database has inbound query-in port', () => {
    ...
    const queryIn = canvasNode.data.ports.inbound.find((p) => p.name === 'query-in');
  ...
  it('data/database has outbound replication-out port', () => {
    ...
    const replOut = canvasNode.data.ports.outbound.find((p) => p.name === 'replication-out');
  ...
  it('messaging/message-queue has inbound publish-in port', () => {
    ...
    const publishIn = canvasNode.data.ports.inbound.find((p) => p.name === 'publish-in');
  ...
  it('messaging/message-queue has outbound consume-out port', () => {
    ...
    const consumeOut = canvasNode.data.ports.outbound.find((p) => p.name === 'consume-out');
