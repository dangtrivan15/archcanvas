...
describe('Feature #541: WebSocket connection management between browser and bridge', () => {
  ...
  describe('Integration: Full connection lifecycle', () => {
    ...
    it('store sendInput() wraps input as JSON and forwards to bridgeConnection.send()', () => {
      const source = fs.readFileSync('src/store/terminalStore.ts', 'utf-8');
      expect(source).toContain("bridgeConnection.send(JSON.stringify({ type: 'stdin', data: input }))");
    });

    it('onMessage callback handles server message types (stdout/stderr/ready) and client types', () => {
      const source = fs.readFileSync('src/store/terminalStore.ts', 'utf-8');
      // Server sends 'stdout'/'stderr'/'ready'; client also handles 'output'/'status'/'error'/'exit'
      expect(source).toContain("type === 'output'");
      expect(source).toContain("type === 'stdout'");
      expect(source).toContain("type === 'stderr'");
      expect(source).toContain("type === 'ready'");
      expect(source).toContain("type === 'status'");
      expect(source).toContain("type === 'error'");
      expect(source).toContain("type === 'exit'");
    });
