...
import { saveLastProject, clearLastProject } from '@/core/project/projectPersistence';
...
export const useProjectStore = create<ProjectStoreState>((set, get) => ({
  ...
  openProjectFolder: async () => {
    ...
    // Clear the previous canvas so old nodes don't linger
    ...
    // Persist the directory handle so the project survives page reloads
    saveLastProject(result.manifest.name, dirHandle).catch((err) => {
      console.warn('[ProjectStore] Failed to persist project handle:', err);
    });
  ...
  closeProject: () => {
    clearLastProject().catch((err) => {
      console.warn('[ProjectStore] Failed to clear persisted project:', err);
    });
    set({
      manifest: null,
      directoryHandle: null,
...
function buildEmptyProjectDialogInfo(
  ...
  return {
    ...
    onUseAI: () => {
      ...
      // Set an init prompt that will be auto-sent once the bridge connects
      useTerminalStore.getState().setPendingInitPrompt(
        `Analyze the codebase in this project folder and create an architecture diagram. ` +
        `Use the archcanvas MCP tools (init_architecture, add_node, add_edge) to build the diagram.`,
      );
