...
export async function autoRegisterMcpConfig(
  ...
  try {
    ...
    if (existing !== null) {
      const desired = buildArchcanvasEntryBrowser();
      const current = existing.content.mcpServers[ARCHCANVAS_SERVER_KEY];

      // Skip write if the entry already matches — avoids triggering file watchers
      if (current && JSON.stringify(current) === JSON.stringify(desired)) {
        return { written: false, created: false, merged: false };
      }

      existing.content.mcpServers[ARCHCANVAS_SERVER_KEY] = desired;
