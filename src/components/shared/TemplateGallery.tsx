...
function TemplateGalleryContent({ onClose }: { onClose: () => void }) {
   ...
   */
  const processArchcFile = useCallback(async (data: Uint8Array, fileName: string) => {
    if (!isArchcFile(data)) {
    ...
    const archBytes = Architecture.encode(
      Architecture.create(archProto),
    ).finish();

    const defaultName = graph.name && graph.name !== 'Untitled Architecture'
      ? graph.name
  ...
  const handleConfirmImport = useCallback(async (name: string, category: string) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isDragOver && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-blue-600/10 rounded-lg pointer-events-none" data-testid="template-gallery-drop-zone">
      ...
      {deleteConfirm && (
        ...
        >
          <div className="bg-[hsl(var(--background))] border border-[hsl(var(--border))] rounded-lg shadow-xl w-[360px] max-w-[90vw] p-5" data-testid="template-delete-confirm-dialog">
